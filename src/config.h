...
struct HostConfig {
  ...
  std::string funding_seg; // 资金费率输入段(FundingShm,订阅者写):必需, 连不上即启动失败
...
inline void print_usage(const char* argv0) {
  std::fprintf(stderr,
      "用法: %s <okx_depth_seg> <okx_trade_seg> <bn_booktick_seg> <funding_seg> <out_seg> [cpu] [poll_us] [--dump <jsonl>]\n"
      "  轮询 gconf shm(OKX DepthBoard+TradeRing / BN BookTickBoard)+ funding 段 → tick_feat 引擎 → 写 FactorBoard。\n"
      "  注: mid/imb1/imb5/spread 全用 OKX DepthBoard(5 档)。\n"
      "  funding_seg: 资金费率输入段(funding_sub 写);必需, 连不上即启动失败(okx/bn_funding 透传进 bit14/15)。\n"
